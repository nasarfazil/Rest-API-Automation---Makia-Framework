...
public class RESTAssuredBase extends PreAndTest {
	...
	public static RequestSpecification setLogs() {
		...
		try {
			log = RestAssured.given().log().all().contentType("application/json");
		} catch (Exception e) {
			e.printStackTrace();
		}
		return log;
	...
	public static Response postWithBodyAsFile(String body) {

		return setLogs().body(body).post();
	}
	...
	public static Response postWithAuthHeader(String id,String jsonObject, String URL) {

		return setLogs().header("authorization",id).body(jsonObject).post(URL);
	}
